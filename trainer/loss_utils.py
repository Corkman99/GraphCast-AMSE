...
LossParameter = namedtuple(
    "LossParameter", ("name", "varname", "dtype", "default", "help")
)
...
Parameters = (
    LossParameter(
        ...
        str,
        None,
        "File containing non-default variable and level weights",
    ),
    LossParameter(
        "--wind-speed",
        "wind_speed",
        bool,
        False,
        "Add wind speed variable to loss function",
    ),
    LossParameter(
        "--time-bias",
        "time_bias",
        bool,
        False,
        "Add time-averaged term to loss function",
    ),
    LossParameter(
        "--mean-bias",
        "mean_bias",
        bool,
        False,
        "Add global mean bias term to loss function",
    ),
    LossParameter(
        "--spectral-amse",
        "spectral_amse",
        bool,
        False,
        "Compute loss in spectral space, with correlation/ampltidue adjustment",
    ),
    LossParameter(
        "--mae",
        "mae",
        bool,
        False,
        "Compute loss with mean absolute error rather than MSE",
    ),
)
        ...
        if p.dtype is bool:
            ...
            if p.default is False:
                arggroup.add_argument(
                    p.name, action="store_true", dest=p.varname, help=p.help
                )
            elif p.default is True:
                arggroup.add_argument(
                    p.name, action="store_false", dest=p.varname, help=p.help
                )
            else:
                raise ValueError(
                    f"Invalid default {p.default} for boolean parameter {p.name}"
                )
        else:
...
config_dict = (
    {}
)  # Dictionary for loss-function configuration, to be populated by parse_arguments
...
def make_loss_new(
    ...
    model_latitude, model_longitude = forecast.generate_model.get_model_coords(
        model_config
    )
    ...
    # this part is still not so clear, are we using the provided files?
    ...
    # this can all be ignored, as we provide the loss function directly (not using this wrapper)
    ...
    if spectral_amse and mae_error:
        raise ValueError(
            "Cannot use both AMSE and MAE error calculations simultaneously"
        )
    ...
    if spectral_amse:  # Use spectral AMSE
        if not silent:
            print(
                "Computing loss function in spectral space, with amplitude/correlation adjustment"
            )
        # spectral AMSE is not compatible with time/mean bias loss
        if time_bias or mean_bias:
            raise (
                ValueError(
                    "Spectral AMSE loss is not compatible with time-bias or mean-bias loss terms"
                )
            )
        ...
        if not silent:
            print("Generating spectral coefficients")
        ...
        if not silent:
            print("Using mean absolute error for loss function")
        if time_bias or mean_bias:
            raise (
                ValueError(
                    "MAE loss is not compatible with time-bias or mean-bias loss terms"
                )
            )
...
def spectral_adj_loss(
    prediction: "xarray.Dataset",
    analysis: "xarray.Dataset",
    ...
    loss = sum(
        per_variable_weights.get(v, 1.0) * loss_by_var[v] for v in mse_by_var.data_vars
    )
...
def spatial_loss(
    ...
    mse = (
        (
            ((diffs**2).mean(dim="lon") * latitude_weights).mean(dim="lat")
            * adj_level_weights
        )
        .sum(dim="level")
        .mean(dim=("time", "batch"))
    ...
    if compute_time_bias and diffs.time.size > 1:
        ...
        mse_time_bias = (
            (
                (
                    ((diffs.mean(dim="time") ** 2).mean(dim="lon")) * latitude_weights
                ).mean(dim="lat")
                * adj_level_weights
            )
            .sum(dim="level")
    ...
    if compute_mean_bias:  # Take mean in lat/lon before squaring
        mse_mean_bias = (
            (
                ((diffs.mean(dim="lon") * latitude_weights).mean(dim="lat") ** 2)
                * adj_level_weights
            )
            .sum(dim="level")
            .mean(dim=("time", "batch"))
...
def mae_loss(
    ...
    mae = (
        (
            (np.abs(diffs).mean(dim="lon") * latitude_weights).mean(dim="lat")
            * adj_level_weights
        )
        .sum(dim="level")
        .mean(dim=("time", "batch"))
...
def unwrap_mean(f):
    ...
    def wrapped_f(*args):
        ...
        return xarray_tree.map_structure(
            lambda x: xarray_jax.unwrap_data(x.mean(), require_jax=True), val
        )
...
def make_loss(
    ...
    compute_time_bias=False,
    compute_mean_bias=False,
):
    ...
    def my_loss(prediction, targets):
        ...
        if compute_time_bias and diffs.time.size > 1:
            ...
            mse_time_bias = (
                (
                    (
                        ((diffs.mean(dim="time") ** 2).mean(dim="lon"))
                        * latitude_weights
                    ).mean(dim="lat")
                    * adj_level_weights
                )
...
def make_amse_loss(
    checkpoint_path, stats_path, per_variable_weights, compute_wind_speed=True
):
    import xarray as xr
    import numpy as np
    from trainer.spectrum import generate_spectral_coefs, sht_eval
    from forecast.generate_model import get_model_coords, load_model
    from graphcast.losses import normalized_latitude_weights
    from trainer.loss_utils import spectral_adj_loss, derived_variables

    # Needed objects:
    (model_config, task_config, _) = load_model(checkpoint_path)
    diffs_stddev_by_level = xr.open_dataset(stats_path + "diffs_stddev_by_level.nc")
    stddev_by_level = xr.open_dataset(stats_path + "stddev_by_level.nc")

    model_latitude, _ = get_model_coords(model_config)
    latitude_weights = normalized_latitude_weights(
        model_latitude.rename(latitude="lat")
    )
    latitude_weights = latitude_weights / latitude_weights.mean()
    input_variables = list(task_config["input_variables"])  # type: ignore
    target_variables = list(task_config["target_variables"])  # type: ignore

    levels = np.array(task_config["pressure_levels"])  # type: ignore
    level_weights = xr.DataArray(levels, dims=("level"), coords={"level": levels})
    level_weights = level_weights / level_weights.sum()
    ...
    # Generate spectral coefficients
    leg_coef = generate_spectral_coefs(model_latitude)
    sht_forward = lambda f: sht_eval(leg_coef, f)
