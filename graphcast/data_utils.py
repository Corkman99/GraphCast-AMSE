...
def add_tisr_var(data: xarray.Dataset, tsi_data) -> None:
    """Adds TISR feature to `data` in place if missing.

    Args:
      data: Xarray dataset to which TISR feature will be added.
      tsi_data: function to retrieve tsi data (None for ERA5, solar_radiation.hres_tsi_data for HRES)
    ...
    tisr = solar_radiation.get_toa_incident_solar_radiation_for_xarray(
        data_no_batch, tsi_data=tsi_data, use_jit=True
    )
...
def extract_inputs_targets_forcings(
    ...
    *,
    ...
    dataset = dataset.sel(level=list(pressure_levels))  # select pressure levels
    ...
    if set(forcing_variables) & _DERIVED_VARS:
        print("Adding derived variables to dataset.")
        add_derived_vars(dataset)
    if set(forcing_variables) & {TISR}:
        print("Adding TISR variable to dataset.")
        add_tisr_var(dataset, tisr_source)
    # not the problem, no Nans in either derived or TISR
