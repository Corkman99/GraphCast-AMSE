...
class Bfloat16Cast(predictor_base.Predictor):
    ...
    def loss(
        ...
        **kwargs,
        ...
        return tree_map_cast(
            (loss, scalars), input_dtype=jnp.bfloat16, output_dtype=targets_dtype
        )
...
def infer_floating_dtype(data_vars: Mapping[str, chex.Array]) -> np.dtype:
    ...
    dtypes = {
        v.dtype for k, v in data_vars.items() if jnp.issubdtype(v.dtype, np.floating)
    }
    if len(dtypes) != 1:
        dtypes_and_shapes = {
            k: (v.dtype, v.shape)
...
def bfloat16_variable_view(enabled: bool = True):
    ...
    * Every time a variable a variable is requested as bfloat16, it will check the
    ...
    if enabled:
        with hk.custom_creator(_bfloat16_creator, state=True), hk.custom_getter(
            _bfloat16_getter, state=True
